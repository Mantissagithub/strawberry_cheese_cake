...
median_blur_ksize = 5  # Reduced kernel size for faster processing
gaussian_blur_ksize = (7, 7)  # Reduced kernel size for faster processing
canny_threshold1 = 20  # Adjusted threshold for faster processing
canny_threshold2 = 70  # Adjusted threshold for faster processing
...
class EuclideanDistTracker:
    ...
    def update(self, objects_rect, direction):
        objects_bbs_ids = []
        for rect in objects_rect:
            ...
            for id, pt in self.center_points.items():
                ...
                # Adjusted distance threshold for tracking
                if dist < 20:
                    self.center_points[id] = (cx, cy)
                    objects_bbs_ids.append([x, y, w, h, depth, centroid, id, direction])
                    same_object_detected = True
                    break

            if not same_object_detected:
                self.center_points[self.id_count] = (cx, cy)
                objects_bbs_ids.append([x, y, w, h, depth, centroid, self.id_count, direction])
                self.id_count += 1

        # Clean up unused center points
        new_center_points = {}
        for obj_bb_id in objects_bbs_ids:
            _, _, _, _, _, _, object_id, _ = obj_bb_id
...
class DirectionPublisher(Node):
    def __init__(self):
        ...
        self.model = YOLO("/home/pradheep/strawberry_cheese_cake/irc_autonomous_ws/src/autonomous_stack/autonomous_stack/best.pt")
        ...
        self.capture= cv2.VideoCapture('/dev/video8')
    ...
    def check_Arrow_Detection(self,distance,direction):
        if not self.distance_published and 50 <= distance <= 70:
                self.msg2.data = int(distance)
                self.distance_threshold.publish(msg=self.msg2)
        ...
        if (
                ...
                self.msg1.data = direction
                self.direction_publisher.publish(msg=self.msg1)
    ...
    def send_cmd_vel(self, ret,frame):
        with open('/home/pradheep/strawberry_cheese_cake/irc_autonomous_ws/src/autonomous_stack/autonomous_stack/cameraMatrix.pkl', 'rb') as f:
            camMatrix = pickle.load(f)
        with open('/home/pradheep/strawberry_cheese_cake/irc_autonomous_ws/src/autonomous_stack/autonomous_stack/dist.pkl', 'rb') as f:
    ...
    # Rest of the processing logic from send_cmd_vel
        ...
        frame, self.border_size, self.border_size, self.border_size, self.border_size, 
        ...
        for contour in contours:
                ...
                if cv2.contourArea(contour) < 750:  # Adjust threshold if needed
                    continue
                ...
                if len(approx) == 7:
                    ...
                    if (30 <= tip_angle1 <= 90 and
                        75 <= base_angle1 <= 120 and
                        75 <= base_angle2 <= 120 and
                        ...
                        if M["m00"] != 0:
                            ...
                            focal_length =( camMatrix[0, 0] + camMatrix[1,1])/2 # Focal length in pixels
                            depth = (((self.REAL_ARROW_WIDTH * focal_length) / object_width_in_pixels  )*self.scaling_factor)/2# Depth in cm
                            #print(depth)
                            # Scaling x and y values
                            scale_x = (cx - frame_center_x) * self.scaling_factor  # Convert to cm
                            scale_y = (cy - frame_center_y) * self.scaling_factor  # Convert to cm
                            if variance != 0 and mean_ratio != 0 and variance < 2 and mean_ratio < 1:
                                detected_arrows.append({
                                    "bbox": (x, y, w, h,depth),
                                    "id": None,  # ID will be assigned by the tracker
                                    "cx": cx,
                                    "cy": cy,
                                    "z": depth,
                                    "x": scale_x,
                                    "y": scale_y,
                                    "direction":direction_finalised,
                                })
                            #detected_arrows.append((x, y, w, h, contour, direction))
                # global boxes_ids
        # Sort arrows by depth (z) to prioritize the closest one
        detected_arrows.sort(key=lambda arrow: arrow["z"])

        # Display the x, y, z values of the closest arrow
        filtered_arrows = [arrow["bbox"] for arrow in detected_arrows]
        
        boxes_ids = tracker.update(filtered_arrows, direction_finalised)
        print(boxes_ids)
        distance = 0
        for boxes in boxes_ids:
                ...
                mid_x = boxes[0] + boxes[2] // 2
                mid_y = boxes[1] + boxes[3] // 2
